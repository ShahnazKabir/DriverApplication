...
import { driverLocationUpdate, getOrderList } from './../../stores/actions';

import urls from '../../stores/url';
...
const driverLocationUrl = urls.baseUrl + urls.sendLocationUrl;

BackgroundTask.define(async () => {
  // Fetch some data over the network which we want the user to have an up-to-
  // date copy of, even if they have no network when using the app
  let orderApi = await AsyncStorage.getItem('orderData');
  orderApi = JSON.parse(orderApi).orderApiData;
  navigator.geolocation.getCurrentPosition((position) => {
    orderApi.driverLattitude = position.coords.latitude;
    orderApi.driverLongitude = position.coords.longitude;
  })
  await AsyncStorage.setItem('orderData', JSON.stringify(orderApi));
  ...
  axios.put(driverLocationUrl, orderApi, headers)
    .then((response) => {
...
class DriverNavigationBody extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  fetchCurrentPosition() {
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      if(this.props.orderData.region.latitude && this.props.orderData.region.longitude) {
        if(haversine(myPosition, this.props.orderData.region, {unit: 'meter'}) <= 10) {
          Snackbar.show({
  ...
  async fetchDirection (currentLocation) {
    let orderApi = this.props.orderData.orderApiData;
    navigator.geolocation.getCurrentPosition((position) => {
      orderApi.driverLattitude = position.coords.latitude;
      orderApi.driverLongitude = position.coords.longitude;
    })
    await AsyncStorage.setItem('orderData', JSON.stringify(orderApi));
    this.props.driverLocationUpdate(orderApi);
  ...
  calculateBearing(coords) {
    if(this.state.coords.lentgh >= 2) {
      const lat1 = coords[0].latitude;
      const lon1 = coords[0].longitude;
      const lat2 = coords[1].latitude;
      const lon2 = coords[1].longitude;
      const bearingAngle = Math.atan2(Math.cos(lat1)*Math.sin(lat2)-Math.sin(lat1)* Math.cos(lat2)* Math.cos(lon2-lon1),
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={{flex: .1}}>
          ...
          <Footer
            ...
            startEvent={this.handleUrl}
            submitEvent={ this.props.submitOrder}
            orderValue={this.props.orderData.orderApiData}
          ></Footer>
        </View>
      </View>
    );
...
export default connect(
  mapStateToProps, {driverLocationUpdate, getOrderList}
