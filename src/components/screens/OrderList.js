  ...
  import { getOrderList, clearAuthStore, driverStatus } from './../../stores/actions';
  ...
  class OrderListPage extends Component {
      ...
      this.props.getOrderList()
        .then(response => {
          if(this.props.errorStatus == 401) {
            ...
            let orderListData = this.props.listData;
            orderListData = orderListData.filter( el => el.status !== 4 );
            console.log(orderListData);
            this.setState(state => {
              const order = orderListData.map((orderData, index) => {
                const orderObj = {
                  id: null,
                  region: {
                    latitude: null,
                  ...
                  orderObj.region.latitude = orderData.lattitude ? orderData.lattitude : null;
    ...
    redirect = async (order) => {
      const check = await AsyncStorage.getItem('orderData');
      try {
      ...
      if(this.state.order.find(o => o.status === 3) && order.status !== 3) {
        Snackbar.show({
  ...
  export default connect(
    mapStateToProps, {getOrderList, clearAuthStore, driverStatus}
  )(OrderListPage);